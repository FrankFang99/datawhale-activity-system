/**
 * 5 维评分引擎单测（切片 2）
 * 覆盖 RC-001 ~ RC-005 全部规则分支 + 等级映射 + 边界条件
 *
 * 测试输入参考 PRD §5.1 v1 现行版
 */
...
describe('RC-002 招募能力（20 分）', () => {
  it('4 渠道（含 暂无）→ 0 分（按非"暂无"渠道数）', () => {
    // 暂无可与其他选项同时存在（v1 简化：取非"暂无"数）
    const r = scoreApplication(baseInput({ recruitChannel: ['暂无', '社群', '公众号', '高校社团'] }));
    // 3 非"暂无"渠道
    expect(r.RC002.score).toBe(20);
  ...
  it('2 渠道 → 14 分（良好）', () => {
    ...
    expect(r.RC002.score).toBe(14);
  });
  it('1 渠道 → 8 分', () => {
    const r = scoreApplication(baseInput({ recruitChannel: ['社群'] }));
    expect(r.RC002.score).toBe(8);
  });
  it('0 渠道 → 0 分', () => {
...
describe('RC-005 活动价值（20 分）', () => {
  it('空 → 0 分', () => {
    const r = scoreApplication(baseInput({ motivation: '', participantValue: '' }));
    expect(r.RC005.score).toBe(0);
  });
  it('高质量：6 关键词全命中 + 长度≥60 → 满分', () => {
    const r = scoreApplication(baseInput({
      motivation: '目标是降低 AI 工具使用门槛；让零基础同学快速上手；通过实操让同学们掌握大模型应用；搭建本地 AI 交流社群，助力就业；坚持共学打卡',
      participantValue: '',
    }));
    expect(r.RC005.score).toBeGreaterThanOrEqual(15);
    expect(r.RC005.hitKeywords).toBeDefined();
  });
  it('中等：3-4 关键词 → 中等分', () => {
    const r = scoreApplication(baseInput({
      motivation: '目标是推动 AI 教育进校园；通过实操让同学们掌握 AI 工具；搭建本地 AI 交流社群。',
      participantValue: '',
    }));
    expect(r.RC005.score).toBeGreaterThanOrEqual(8);
...
describe('总分 + 等级映射', () => {
  ...
  it('A 级（75-89）= 良好 → 自动 CONFIRMED', () => {
    const r = scoreApplication(baseInput({
      venueStatus: '已确定',
      recruitChannel: ['社群', '公众号'],
      experience: '组织过 1 次校内 AI 分享会，参与人数 50+',
    }));
